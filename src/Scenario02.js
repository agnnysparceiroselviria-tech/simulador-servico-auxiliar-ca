export const Scenario02 = {
  id: 'scenario2',
  name: 'CENÁRIO 2',
  description: 'FONTE DA UG-02 INDISPONÍVEL DEVIDO À MODERNIZAÇÃO',

  engineProfile: {
    operationalUnits: ['UG01', 'UG11', 'UG12'],
    incomingSources: {
      107: 'UG01',
      108: 'UG11',
      109: 'UG12',
...
Scenario02.generators?.forEach(shiftItemX);
Scenario02.relays?.forEach(shiftItemX);
Scenario02.externalSources?.forEach(shiftItemX);
Scenario02.distributionPanels?.forEach(shiftItemX);
Scenario02.transformers?.forEach(shiftItemX);
Scenario02.groundSwitches?.forEach(shiftItemX);
Scenario02.incomingBreakers?.forEach(shiftItemX);

Scenario02.stepUpBranches?.forEach((branch) => {
...
Scenario02.panels?.forEach((panel) => {
  shiftItemX(panel);
