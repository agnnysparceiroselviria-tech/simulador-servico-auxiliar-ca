// UGSimulator.js - versão corrigida 22/09/2026
// Mantém desenho da UG, animação de rotação e painel de Partida / Parada.
...
export const UGSimulator = {
  ...
  render() {
    ...
    if (onStartStop) {
      ...
      const qcVoltageValue = this.root.querySelector('#qcVoltageValue');
      if (qcVoltageValue) {
        qcVoltageValue.textContent = `${voltagePercent}%`;
      }

      // Compatibilidade: versões anteriores possuíam uma barra visual de tensão.
      // Se ela não existir no HTML atual, o render continua normalmente.
      const qcVoltageBar = this.root.querySelector('#qcVoltageBar');
      if (qcVoltageBar) {
        qcVoltageBar.style.width = `${voltagePercent}%`;
      }
